...
/**
 * 覆写 LinkMovementMethod onTouch 再点击span的时候，textview onclick不响应
 * Created by Administrator on 2016/3/18.
 */
public class TextMovementMethod extends BaseMovementMethod {
    ...
    public TextMovementMethod(){
        mTextViewBgColorId = DEFAULT_COLOR;
        mClickableSpanBgClorId = DEFAULT_COLOR;
    }

    public TextMovementMethod(int clickableSpanBgClorId){
        mClickableSpanBgClorId = clickableSpanBgClorId;
        mTextViewBgColorId = DEFAULT_COLOR;
    }

    /**
     * 可以进行自定义点击的颜色，点击的颜色，背景颜色
     * @param clickableSpanBgClorId
     * @param textViewBgColorId
     */
    public TextMovementMethod(int clickableSpanBgClorId, int textViewBgColorId){
        mClickableSpanBgClorId = clickableSpanBgClorId;
    ...
    public boolean onTouchEvent(TextView widget, Spannable buffer,
        ...
        if(action == MotionEvent.ACTION_DOWN){
            ...
            if(mClickLinks.length > 0){
                ...
                //设置点击区域的背景色
                mBgSpan = new BackgroundColorSpan(ApplicationConfig.getContext().getResources().getColor(mClickableSpanBgClorId));
