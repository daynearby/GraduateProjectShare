...
public class Crop {
    ...
    //选择拍照
    public static final int PIC_FROM_CAMERA = 1993;
    // 选择本地图库
    public static final int PIC_FROM_LOCAL = 1990;

    //存储的文件名
    public static final String imgFileName = "camera.jpeg";
     ...
     */
    public static void pickImage(Activity activity, int Type, boolean Big) {


        if (PIC_FROM_CAMERA == Type) {//选择相机
            Intent intentCamera = new Intent(MediaStore.ACTION_IMAGE_CAPTURE);
            intentCamera.putExtra(MediaStore.EXTRA_OUTPUT, Uri.fromFile(new File(CropUtil.CreateImageFile(activity), imgFileName)));

            try {
                activity.startActivityForResult(intentCamera, PIC_FROM_CAMERA);

            } catch (ActivityNotFoundException e) {
                e.printStackTrace();
                Toast.makeText(activity, "相机出错", Toast.LENGTH_SHORT).show();
            }
            ...
            try {
                ...
                e.printStackTrace();
                Toast.makeText(activity, "无效的图片", Toast.LENGTH_SHORT).show();
            }
